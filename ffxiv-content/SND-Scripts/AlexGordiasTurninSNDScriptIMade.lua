--OTHER DEPENDENCY: DOESN'T WORK FOR CHARACTERS IN THE MAELSTROM YET. SORRY.
...
--ADDON DEPENDENCY: VISLAND, USED TO PATH TO BOTH SABINA AND GC HEADQUARTERS
...
--UPDATE THIS TO: 'Storm' for Limsa / 'Flame' for Ul'dah (it's the word in the Personnel Officer/Quartermaster's name) [LIMSA IS NOT ROUTED]
CharacterGC = 'Flame'

--Set item ID for the GC seal for the GC we belong to. Have to do this before we do anything else.
if
CharacterGC == 'Serpent' then SealID = 21
elseif
CharacterGC == 'Storm' then SealID = 20
elseif
CharacterGC == 'Flame' then SealID = 22
else SealID = 28 --if improperly configured, checks against Poetics, which you're guaranteed to have less than 20k of => never try to make a purchase to be safe from bad situations. Don't do this.
end

--Set the item to purchase. Currently supported: 'Materiel4', 'Materiel3', 'Cashmere' (you don't want materiel container 3.0, trust me)
...
yield("/echo Item to Purchase is improperly configured. Please edit line 26. Quitting...")
yield("/snd stop")
end

--tarnished gordian item ids, hardcoded for all later references
LensID = 12674
ShaftID = 12675
CrankID = 12676
SpringID = 12677
...
ItemExchangeCount = 0 --this is "how many items are currently in our inventory to hand over to the personnel officer". assumes zero.

--generic functions for purchasing items, thankfully the pcall doesn't change across menus
--I think I could actually have only one function here named PurchaseItem, and take two parameters in, where the other is the Gordian item name, but I'm real dumb and stupid and lazy
...
function PurchaseItemsFromSabina()
  ...
  PurchaseSpringItem(9)
  else end
  
  if SpringCount > 3 then
  PurchaseSpringItem(10)
  else end
  
  if SpringCount > 3 then
  PurchaseSpringItem(11)
  else end
  
  --pedals
  if PedalCount > 2 then
  PurchasePedalItem(12)
  else end
  
  if PedalCount > 2 then
  PurchasePedalItem(13)
  else end
  
  if PedalCount > 2 then
  PurchasePedalItem(14)
  else end
  ...
  if SpringCount > 3 then
  PurchaseSpringItem(6)
  else end
  
  if SpringCount > 3 then
  PurchaseSpringItem(7)
  else end
  ...
  if
  CharacterGC == 'Serpent' then GoToGridaniaGCNPC()
  elseif
  CharacterGC == 'Storm' then
  --GoToLimsaGCNPC()
  yield("I HAVEN'T PATHED OUT LIMSA YET. SORRY!")
  yield("/snd stop")
  elseif
  CharacterGC == 'Flame' then GoToUldahGCNPC()
  else
  yield("/echo Grand Company name is incorrectly. I'm looking for Serpent/Storm/Flame")
  end  
end
...
--hell yeah three distinct copy and pasted functions
...
function GoToUldahGCNPC()
  ...
  --run from aetheryte plaza to gc turnin npc
  yield("/visland moveto -132 -2 -155") --entrance of the Weird Aetheryte Nook, uldah doesn't even have guards on it, how do they control the capitalists smuggling things in (well...)
  yield("/wait 5")
...
--THIS IS NOT A REAL ROUTE. I JUST COPYPASTED GRIDANIA. DO NOT USE THIS.
...
function TurnInGCItems()
  ...
  for i = 1, ItemExchangeCount, 1 do
	...
	--yes already means I do not have to care about this, but splitting the waits in half is no burden and I'm trying to futureproof this in case I ever send it to anyone else
...
function SpendGCSeals()
  ...
  then
    yield("/pcall GrandCompanyExchange true 2 1 0 0 0 0 0 0 0") --change tabs to Materiel
    yield("/wait 2")
    if CharacterGC == 'Flame' --dammit Nanamo. my code could at least pretend to be elegant before you :(
    then 
  ...
  yield("/echo Item to Purchase is improperly configured. Please edit line 26. Also, you shouldn't have managed to ever see this message because there's an earlier one you failed to hit. Quitting, but very confusedly...")
  yield("/snd stop")
  end
...
yield("/echo Somehow, we managed to get into the quitting loop while we still had bolts left. This should not happen, feel free to PM me") --if we ever see this I have to figure out if I can exploit this behavior somehow
end