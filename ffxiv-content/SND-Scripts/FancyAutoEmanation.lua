--REQUIRED SETUP: HAVE EMANATION (EXTREME) UNSYNCED SELECTED IN THE DUTY FINDER BEFORE STARTING THIS BOT
...
--DEPENDENCIES:
--YESALREADY is assumed, but should theoretically not be mandatory. however, I have not tested without it -Unfortunate
...
--Unfortunate's dubious "improvements":
...
--tank job to farm achievement on. if set to a nontank job or blank, bot will loop forever
JobVariable = 'PLD'
...
--if you set this to "true", the bot will try to set up the proper Duty Finder settings itself. I can't get the damn thing to work. Feel free to try to fix it yourself, dear future corder.
...
--20240408 added this, defaulted to not doing it because I can't get the damned thing to work
--IF THIS VARIABLE IS SET TO FALSE (AS IT IS BY DEFAULT), YOU NEED TO START THE BOT WITH EMANATION (EXTREME) UNSYNCED SELECTED. I ALREADY SAID THIS ONCE BUT I'M SAYING IT AGAIN
if TryToAutoSetup then 
  ...
  --(the other way to make that happen is specifically to toggle FROM restricted TO unrestricted party, but we do not know the user's start state because there's no funciton for that.)
  yield("/pcall ContentsFinder true 12 1") --this hits the "Clear Selection" button, even if you have zero things selected so that button is greyed out. sketchy! but hell, yesalready does worse every day.
  yield("/wait 1")
...
--repair your gear if it can be repaired. code stolen from flug's diadem, but I think they may have borrowed it from someone else. we really oughta make a canonical SND library, oughtn't we?
...
--code stolen from flug to enter Diadem, but changed
--"While we are not queueing, try to queue" (20230328 Unfortunate added the DF opening/joining to this loop condition)
...
while GetCharacterCondition(26) do
  ...
  --todo: divine denial is not properly even triggering the vril thing
...
if OpenCoffers then
  ...
  yield("/wait 1") --make sure loot hits inventory
end
...
if JobAchievement < 9000 then --this means if it's been defined above earlier that we're actually here for tank farming. otherwise, we won't worry about it.
  ...
  --if we just started the bot, report out where we're at
  if FirstRun == 1 then
  ...
  if TotalCount > 699 then
    CompletionVariable = true --for now, 700 kills is the last achievement. this has to be updated by hand if they add more in DT. sorry.
  end
end
...
elseif JobVariable == 'PLD' then
  ...
  yield("/echo The job variable (on line 16 of the script) is not set to a tank job. Achievement tracking is not activated and this bot will run until you stop it manually.")
end
...
--this variable will be set to true when you complete a tank farm achievement and used to to stop the bot. if you're not on a tank job, this bot will run forever until you stop it.
