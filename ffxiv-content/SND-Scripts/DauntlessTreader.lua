--please note: after various autohook rewrites, the creation of vnavmesh, and whatever updates came with Dawntrail and onward that I missed, this may or may not work anymore... but you're welcome to try!

--Instance, repair, and move loop code reused from flug, who I believe borrowed some or all of it from earlier existing SND community scripts
--Fishing actions, autohook and visland presets, and various bugs added by me. Feel free to reuse any or all parts!
...
--Things I would still want to implement before I call this feature-complete:
...
--2) People who can't repair their own gear won't repair their gear here, and we have code to talk to a Mender anyway... but nobody else seems to care about that, just level a crafter.
...
--20240317 note: this will never be fixed, because I finished the achievement. feel free to steal the logic for yourself and implement.
...
--After one catch of Ashfish or Ghost Faerie, use Surface Slap on them and switch to "UmbralTreader2". THIS DOES NOT WORK, AND IT'S AN AUTOHOOK LIMITATION AS OF MARCH 2024. MAYBE IT WORKS NOW? GIVE IT A SHOT I GUESS.
...
--Make sure we're a Fisher, leave the Diadem if we're in it, configure Autohook presets, and get to Aurvael's desk
function InitialStartup()
  ...
  --Make sure we're a Fisher, since we can't guarantee everyone's gearset is named identically to use "gs equip" text command
  ...
  if (not IsInZone(886)) and (not IsInZone(939)) then
    ...
    --Make sure that the aetheryte has loaded too, just in case. This is three conditions because I have no clue how nil works in lua in SND
    ...
    --Hardcoded: rounded output of GetObjectRawXPos('Aetheryte'), because the aetheryte is huge so I don't actually care about the seven decimals of precision and I'm pretty sure it's not moving anytime soon
    AetheryteX = -64
    AetheryteY = 11
    AetheryteZ = 44
  ...
  --Therefore, if we get here, we are already in the Firmament, so we do nothing. vnavmesh can make this fancier theoretically to support starting in Firmament away from entrance/desk.
  else
  ...
  DeleteAllAutoHookAnonymousPresets() --Cleans up after any previous iterations of this very script, used when testing, safe to leave in here all the time
...
function EnterDiadem()
  ...
  if IsInZone(886) then --If we're in the Firmament... (This should, theoretically, not be necessary. But I'm very, very suspicious.)
    ...
    if GetDistanceToObject('Aurvael') > 6.9 then --Seven is the standard interaction distance, which I did not know until I started testing this
      yield("/visland exectemponce " .. AurvaelRoute) --Move to Aurvael. There's no reason for this to be a variable, but hey, maybe you like to start from the scrip-spending NPC's desk, you can modify this yourself to do that.
  ...
  --Repair your gear outside of Diadem if it can be repaired (That's what the 99's for, this is "repair everything at 99% or lower durability")
    ...
    while GetCharacterCondition(34, false) and GetCharacterCondition(45, false) do
      ...
      --if duty finder is up, enter Diadem. else, in order, target aurvael, talk to aurvael, select to enter, select yes to enter. Many of these are skippable with textadvance/yesalready, thus this boondoggle.
      ...
      --after doing a thing, wait half a second
      yield("/wait 0.5")
    end
    
    --When we're out of here, we're queued up. Now wait until we start entering, then wait until we're done entering, then wait 3
...
--move to the general location that we'll be using to fish the whole time, but not at any of the specific fishing spots we use (meaning no amiss-timer issues)
function InitialDiademEntryMove()
  ...
  --Record our entry time. Instance timer is 180 minutes = 3 hours > 40 minutes = 2400 seconds and then leave to dodge the amiss timer (I coded the movement to dodge amiss status on purpose, then it still didn't. "yeah rip.")
  ...
  if (BalloonBugCount < 100 or RedBalloonCount < 100 or CraneFlyCount < 100) then
    ...
    if DebugVariable then
	    yield("/echo Debug: Low on bait. Restocking bait.")
	  end
...
--restock bait, as the name suggests
function RestockBait()
  if DebugVariable then
    yield("/echo Debug: Going to the Mender and restocking bait.")
  end
  ...
  --There's not strictly a need to exit the Mender menu since the next step could be unmounted movement instead of mounted, but you can't mount up while you're in the chat menu and mounting is faster
...
--Wait for the weather to change away from Snow. We don't care what it is as long as it's not Snow, since in the Diadem the only options are Snow and some flavor of Umbral
--Diadem weather alternates between 10 minutes of snow -> 10 minutes of something Umbral (I don't know if it's actually "pick 1/4 at random" but I don't care because we handle all four just fine.)
...
function MoveToFishingPoint()
  ...
  --Select a new fishing point to dodge amiss. this is currently NOT working and just overcomplicating everything lol
  OldFishingPoint = NewFishingPoint
  
  while OldFishingPoint == NewFishingPoint do
    NewFishingPoint = math.random(1,7) --Yes, this *can* theoretically reroll forever at a chance of (1/7) ^ (infinity). Did you know every game of Tetris is guaranteed to end when you get enough Z pieces in a row?
  end
  ...
  then
    ...
    yield("/echo Something went incredibly wrong with fishing point selection. Random(1,7) yielded a value that was not an integer between one and seven inclusive? Quitting...")
...
function SelectBait()
  ...
  yield("/bait Diadem " .. BaitName) --huh, this SimpleTweaks function doesn't take double quotes around the bait name, just raw text
...
function FishDuringUmbralWeather()
  ...
  while GetCharacterCondition(6) do
    ...
    if DebugVariable then
      ...
      yield("/echo Debug: Mid-animation status: " .. tostring(GetCharacterCondition(42))) --how on earth does this return both true and false? wild.
	  end
  ...
  if DebugVariable then
    yield("/echo Debug: Weather has changed back to Snow and we have stopped fishing. Waiting for the next weather change.")
  end
...
--The purpose of the function immediately above is to make it so the actual "meat" of this script, when run, consists of this single line. Nice.
MainFunctionLoop()