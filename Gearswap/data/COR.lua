...
function user_setup()
	...
	--define other stuff (just this helm, in case I ever have another herc helm for any reason, which I probably won't because it's not 2022.)
    gear.HercHelmFC = { name="Herculean Helm", augments={'"Mag.Atk.Bns."+8','"Fast Cast"+6','MND+8',}}
    ...
    gear.EmpyLegs = {name="Chasseur's Culottes +2"} --11 dt (12 at +3)
...
function init_gear_sets()
    ...
    sets.precast.JA['Snake Eye'] = {} --{legs=gear.RelicLegs,} --12% chance of 0 recast (4% per merit), but inventory
	...
	--FC is 44, cap is 80. I don't really care. anything BLU can use, or THF can use, we can probably use here.
    sets.precast.FC = {
	...
	--this is currently 64 + 27 (ikenga's boots is 5 so that doesn't let me drop a 10, taeon can go to 10 per slot in the big five but those are already that or better... there's really nothing, huh. someday.)
    sets.precast.RA = {
    ...
    --hot shot has good ftp scaling from moonshade so use the aeolian set with moonshade in it
    ...
    --full malignance set is 6 9 5 7 4 = 15 + 12 + 4 = 31 DT. empy legs is +4 over tights, plus murky ring puts us at 45. properly augmented alabaster earring would be optimal here but screw it. full time 50 dt all time.
    ...
    sets.midcast.RA = {
        ...
        right_ring="Murky Ring", --10 dt
        waist="Yemaya Belt", --stp, racc, ratk, I think this is optimal?
		back="Null Shawl",
        }
    ...
    --there's a reasonable argument for not using the oshosi, unsure - I keep it around for snapshot anyway so it's not -1 inventory slot which is really all that matters.
    sets.TripleShot = {
    ...
    sets.engaged = {
        ...
        legs=gear.EmpyLegs, --11 dt (31) (reforge this to +3 when I can, engaged set is important!)
        feet="Malignance Boots", --4 dt (35)
        neck="Iskur Gorget",
		waist="Reiki Yotai", --7 dw
        left_ear="Alabaster Earring", --5 dt (40) and also we need the gear haste with empy legs because full malig is 26 and empy legs are -3 from malig
        right_ear="Eabani Earring", --4 dw
        left_ring="Epona's Ring", --assume this beats petrov, also inventory
