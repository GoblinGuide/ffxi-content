now-- Original: Motenten / Modified: Arislan / absolutely gutted: DACK on 5/20/23 (to replace existing because TH tag was broken)
...
function user_setup()
    ...
    gear.AFFeet = {name="Atrophy Boots +2"} --nope
...
function init_gear_sets()
	...
	--non-thf TH cap is 4, this is 2+2, have one +1 piece, need one more
    sets.TreasureHunter = {
    body="Volte Jupon", --2
	legs=gear.Chironic.THLegs, --2
	}
	...
	--FC cap is 80, occ quicken cap is 10, rdm has 38 native FC => 42+10 from gear needed, this set caps without weapon swap to Crocea Mors (with the exception of Impact, see set below)
	--other slots got HP swaps where possible to prevent me from losing so much hp every time I cast
    sets.precast.FC = {
	ammo="Impatiens", --2 occ (0+2)
	head=gear.AFHead, --16 (16+2)
	body=gear.RelicBody, --15 (31+2)
	hands={name="Nyame Gauntlets",priority=91}, --91 HP, don't need the 8 from gear.LeylineGloves,
	waist={name="Plat. Mog. Belt",priority=200}, --don't need "Witful Belt" 3+3
	legs={name="Nyame Flanchard",priority=114}, --114 HP
	feet={name="Nyame Sollerets",priority=68}, --68 HP
	left_ear={name="Eabani Earring",priority=45}, --don't need the 2 from "Loquac. Earring",
	right_ear="Lethargy Earring", --7 (38+2)
    left_ring="Kishar Ring", --4 (42+2)
    right_ring="Defending Ring", --no reason not to put DT here, i think we're not actually capped
	back="Perimede Cape", --4 occ (42+6)
	}
    ...
    --in total, lose 31 from the head and body and gain 10 from the two pieces I have here + 20 from Crocea Mors that I likely have equipped = 1 short of cap which is ok if not technically optimal
    sets.precast.FC.Impact = set_combine(sets.precast.FC, {
	body="Crepuscular Cloak", --obv
	--legs = "Orvail Pants +1, --8, currently not in wardrobe, we're fine even if it sucks that it's not capped
    ...
    --this is dt capped (38 nyame, 10 dring, 5 cape)
    sets.precast.WS['Savage Blade'] = {
	ammo="Oshasha's Treatise", --fully augmented coiste bodhar beats this someday, theoretically
	head="Nyame Helm", --dt
    body="Nyame Mail", --dt
    ...
    --relative to savage blade set, this is -7 dt from nyame helm +6 from loricate torque, still capped
	...
	--cure potency caps at 50. daybreak randomly has 30 so this set got a lot less complicated and more hardcoded as long as I'm willing to assume Dual Wield, which... todo: make this work for sheol gaol etc
	...
	ammo="Impatiens", --10 spell interrupt rate, why not
    ...
    feet={name="Nyame Sollerets",priority=68}, --68 HP
	neck="Nodens Gorget", --5 (47)
    back="Ghostfyre Cape", --6 (53, and I'm using this in other sets anyway)
	}
	...
	--default enhancing magic set. listed values are duration. composure sets for casting on others below, this is self-casting since I "always" have composure up
	--currently 520+ enh skill and almost everything caps at 500 so I don't need to use vitiation tabard +3 in body for +23 enh skill
	--using ghostfyre cape totals 1.82 * 1.68 = 3.0576 duration multiplier
	--with ambu cape instead it's 2.02 * 1.49 = 3.0098
    sets.midcast['Enhancing Magic'] = {
	main="Colada", --4 aug, costs tp
	sub="Ammurapi Shield", --10 reg, costs tp
	...
	neck="Nodens Gorget", --this is free since it's in my cure pot set
    ...
    left_ring="Stikini Ring +1", --8 mnd, 8 enf skill, 11 macc
	right_ring="Stikini Ring +1", --8 mnd, 8 enf skill, 11 macc, unsure if metamor. ring +1 is better (16 mnd and 11-15 macc)
	...
	--PDT/MDT capped (actually 5 over)
    --probably want a carrier sash if rdm can equip that, or a hearty earring or some dumb thing like that
    sets.idle = {
	ammo="Homiliary", --1 refresh (1)
	head=gear.RelicHead, --3 refresh (4) --vs 7 dt from "Nyame Helm", --check meva here too
    body=gear.EmpyBody, --12 dt (12), 3 refresh (7)
    hands="Nyame Gauntlets", --7 dt (19)
    legs="Nyame Flanchard", --8 dt (27)
    feet="Nyame Sollerets", --7 dt (34)
	neck="Loricate Torque +1", --6 dt (40)
	waist="Carrier's Sash", --elemental resistance
	left_ear="Eabani Earring", --15 evasion, the ear slots are awful for idle sets
	right_ear="Etiolation Earring", --should this be odnowa? more dt but I don't need that unless I have a meva cape/second dr ring
    left_ring="Shneddick Ring +1", --movespeed
    right_ring="Defending Ring", -- 10 DT (50)
	back=gear.AmbuCape.Seraph, --seraph blade cape has 5 DT (55) but I'd rather have some meva at this point apparently
	}
	...
	--current engaged DT is 31 from malignance + 10 DT from DRing + 10 PDT from cape is 51/41 and I always have Shell on myself to cap MDT (Shell V is 26)
    sets.engaged = {
	ammo="Coiste Bodhar", --DA
	head="Malignance Chapeau", --eventually bunzi hat at like r25+, but the other four aren't going anywhere
    ...
    right_ring="Defending Ring", --second Chirich Ring is great and all but I'd rather die less
    ...
    --todo: get the lyco. earring lol
    --todo: swap rdm category 2 merit points out of either immunobreak chance or magic accuracy (no idea if 25 macc is worth 15% immunobreak chance) to put in enspell
    ...
    --elemental affinity does work on enspells, so this set assumes enstone, which will not always be correct but is for Ongo V25
    --(and stone isn't any worse than other elements against most other things you care about, like odin)
    --current stats: 22 gear haste (3 below cap), 31+16 dt (missing 19+5 with Shell V to be capped), enspell +35, earth affinity 20/other affinity 15
    sets.Enspell = {
    ...
    neck="Quanpur Necklace", --5 earth affinity for Ongo purposes
    left_ear="Hollow Earring", --enspell +3
    right_ear="Telos Earring", --a source of multi attack even if I already have Temper
    --right_ear="Lycopodium Earring", --enspell +2
    left_ring="Chirich Ring +1", --subtle blow and store tp
    right_ring="Chirich Ring +1", --not dt which I may or may not care about
    back=gear.AmbuCape.TP, --stp, 10 pdt (31+16)
    }
	...
	neck="Nodens Gorget", --25 HP, and also cure pot +5 which does not matter (sanctity necklace is 35, but also 2 regen, so playing it safe if I get interrupted or something)
	...
	--back=gear.MecistoMantle, --47 HP, but inv slot and also I'm fine to get 500 without it lol
	}
